\section{Einleitung}
    ...
    Die conplement AG ist ein mittelständisches IT-Dienstleister, das individuelle Lösungen in Bereichen
    wie Web, Embedded und Mobile anbietet. Dazu gehört das \acrfull{portal} für den Kunden \acrfull{abus}.
    \acrshort*{abus} ist ein Hersteller von Alarmanlagen, Videoüberwachungs- und Zutrittskontrollsystemen.

    Das \acrshort{portal} dient dazu, solche Geräte zu verwalten und zu warten. Dort kann man den Status
    eines Geräts und ggf. dessen Komponenten ansehen, Gerätenachrichten abonnieren und (falls unterstützt)
    ansehen, ein Gerät aus der Ferne warten und vieles mehr. Es gibt vier Rollen, die das \acrshort{portal}
    in unterschiedlichem Umfang und für unterschiedliche Zwecke nutzen können: \Gls{endnutzer}, \Gls{facherrichter},
    \Gls{ts-gls} und \Gls{pm-gls}. Jede Rolle ist im \hyperref[sec:glossary]{Glossar} detailliert beschrieben.

    Das conplement-Entwicklungsteam vom \acrshort{portal} besteht aus einer Projektmanagerin, einem
    Softwarearchitekten, zwei Fullstack-Entwicklern und einem Auszubildenden (mir), ebenfalls
    als Fullstack-Entwickler tätig. Das Team arbeitet eng mit dem Product Owner von \acrshort{abus}
    zusammen und setzt agile Methoden wie Scrum und Kanban ein, um die Entwicklung effizient zu gestalten.
    ...
    soll ebenfalls Zugriff auf diese Funktionalität haben, aber die Hauptzielgruppe sind die
    \acrshort{ts}.
    ...
    Momentan kann ein \acrshort{ts} nur bei Geräten der Produktlinie \textit{Secoris} die Gerätenachrichten
    ansehen. Dazu muss er jedoch eine Freigabe vom \Gls{facherrichter} oder \Gls{endnutzer} einholen,
    was den Support-Prozess verlangsamt. Außerdem hat die Secoris-Log-Ansicht keine grafische Darstellung
    der Nachrichtenhäufigkeit, was die Analyse erschwert.

    Das neue Feature soll diese Probleme lösen und die Effizienz des technischen Supports verbessern. Dadurch
    können sowohl monetäre als auch nicht-monetäre Vorteile für \acrshort{abus} entstehen, wie in der
    \ref{sec:profitability-analysis} beschrieben.
    ...
    \begin{itemize}
        ...
        \item Dies wird in einem bestehenden Backend passieren, das in C\# mit ASP.NET implementiert ist.
        \item Die Auswertungsansicht muss in das bestehende Frontend von dem \acrshort{portal} integriert werden.
    ...
    \begin{itemize}
        ...
        \item Die Nutzer des neuen Features sind die \acrshort{ts} und \acrshort{pm} von \acrshort{abus}.
    \end{itemize}
    ...
    \begin{itemize}
        ...
        optimiert. Daher wird das neue Feature ebenfall nur für die Desktop-Ansicht
        implementiert.
        \item Diagramme: Die Darstellung der Daten in Diagrammen wird mit einer
        externen Bibliothek umgesetzt und ist nicht Teil des Projekts.
    \end{itemize}
